...
| `deployment.yaml` | 无状态 Deployment，挂载 ConfigMap 覆盖配置；挂载点固定为 `/opt/kkfileview/config/application.properties`（与镜像版本无关）。同时定义启动/就绪/存活探针。 |
...
| `application.properties` | 配置的副本（与 `configmap.yaml` 保持同步；运行时以 ConfigMap 为准）。 |
